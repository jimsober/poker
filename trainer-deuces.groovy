    ...
    if (wild_cards.size() == 1) {
        ...
        //three of a straight flush
          ...
          [9,10],[9,11],[9,12],[9,13]])) {
            correct_strategy = 'Three of a straight flush'
            hold_hand = wild_cards.collect()
            if (max_deuceless_suit_vals2.sort() in [[3,14],[4,14],[5,14,],[3,4],[3,5],[3,6],[3,7],[4,5],[4,6],[4,7],[4,8],\
              [5,6],[5,7],[5,8],[5,9],[6,7],[6,8],[6,9],[6,10],[7,8],[7,9],[7,10],[7,11],[8,9],[8,10],[8,11],[8,12],\
              [9,10],[9,11],[9,12],[9,13]]) {
                alt_hold_hand = wild_cards.collect()
                for (card in hand) {
                    if (card[0][1] in max_suits && card[1] in max_deuceless_suit_vals2) {
                        alt_hold_hand.add(card)
                    }
                }
            }
...
def no_deuces(List hand) {
    ...
    //four of a straight
    else if (rank_vals.size() == 5 && grouped_ranks in [['A':1,'3':1,'4':1,'5':1,'7':1]]) {
        correct_strategy = 'Four of a straight'
        for (card in hand) {
            if (card[1] in [3,4,5,14]) {
                hold_hand.add(card)
            }
            if (card[1] in [3,4,5,7]) {
                alt_hold_hand.add(card)
            }
        }
    }
      ...
      [10,11,12,14],[10,11,13,14],[10,12,13,14],[11,12,13,14]]) {
        correct_strategy = 'Four of a straight'
        if (rank_vals.sort()[1..4] in [[3,4,5,7],[3,4,6,7],[3,5,6,7],[4,5,6,8],[4,5,7,8],[4,6,7,8],[5,6,7,9],\
          [5,6,8,9],[5,7,8,9],[6,7,8,10],[6,7,9,10],[6,8,9,10],[7,8,9,11],[7,8,10,11],[7,9,10,11],[8,9,10,12],\
          [8,9,11,12],[8,10,11,12],[9,10,11,13],[9,10,12,13],[9,11,12,13],[10,11,12,14],[10,11,13,14],\
          [10,12,13,14],[11,12,13,14]]) {
            for (card in hand) {
                if (card[1] in rank_vals.sort()[1..4]) {
                    alt_hold_hand.add(card)
                }
